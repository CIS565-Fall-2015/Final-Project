...
void runCuda() {
	...
	if (iteration < renderState->iterations) {
		...
		// execute the kernel
		int frame = 0;
		terrain(pbo_dptr, frame, iteration);
...
bool init() {
    ...
    window = glfwCreateWindow(width, height, "Complex Procedural Terrain", NULL, NULL);
    if (!window) {
        glfwTerminate();
        return false;
    }
...
void mainLoop() {
    while (!glfwWindowShouldClose(window)) {
        ...
        string title = "Complex Procedural Terrain | " + utilityCore::convertIntToString(iteration) + " Iterations";
